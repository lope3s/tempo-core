...
class UserController {
    async storeUser(req: Request, res: Response) {
       ...
       if (!validateBody.valid) return res.status(400).json({
       ...
       try {
           ...
           if (user) return res.status(409).json({error: 'E-mail already registered.'})

           const createdUser = await createUser(validateBody.obj)
            
           return res.status(201).json({message: 'User created.', id: createdUser.insertedId})
       } catch(error: any) {
           console.log(error)
